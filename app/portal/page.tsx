...
// Firebase keys cannot contain dots — encode them as a safe substitute
function toFirebaseKey(username: string) {
...
export default function PortalLoginPage() {
  ...
  async function handleSignIn(e: React.FormEvent) {
    ...
    const firebaseKey = toFirebaseKey(trimmed)
    if (!trimmed) {
    ...
    try {
      const workerRef = ref(db, `workers/${firebaseKey}`)
      ...
      await update(ref(db, `workers/${firebaseKey}`), { sessionToken: token })
      localStorage.setItem("portal_user", firebaseKey)
  ...
  async function handleSignUp(e: React.FormEvent) {
    ...
    const firebaseKey = toFirebaseKey(trimmedUsername)
